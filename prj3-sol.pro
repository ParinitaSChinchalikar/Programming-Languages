...
items_total1([], 0).
items_total1([order_item(_, _, N, P)|T], Total) :-
    items_total1(T, Rest),
    Total is Rest + N * P.
...
items_with_category([Item|Rest], Category, [Item|CategoryItemsRest]) :-
    Item = order_item(_, Category, _, _),
    Category \= unknown,
    items_with_category(Rest, Category, CategoryItemsRest).
items_with_category([_|Rest], Category, CategoryItemsRest) :-
    items_with_category(Rest, Category, CategoryItemsRest).
...
expensive_item_skus([order_item(SKU, _, _, Price)|Rest], Threshold, [SKU|ExpensiveSKUs]) :-
    Price > Threshold,
    expensive_item_skus(Rest, Threshold, ExpensiveSKUs).
expensive_item_skus([_|Rest], Threshold, ExpensiveSKUs) :-
    expensive_item_skus(Rest, Threshold, ExpensiveSKUs).
...
left_plus(PlusExpr, LeftPlusExpr) :-
    % Base case: an atomic is already a left plus expression
    atomic(PlusExpr),
    LeftPlusExpr = PlusExpr.

left_plus(PlusExpr, LeftPlusExpr) :-
    % Base case: an invalid plus expression is the left identity
    % element for the '+' operator
    PlusExpr = 0 + _,
    LeftPlusExpr = PlusExpr.

left_plus(PlusExpr, LeftPlusExpr) :-
    % Recursion case: split PlusExpr into A + B and recurse into A
    PlusExpr = A + B,
    (atomic(A) ->
        LeftA = A
    ;
    ...
    % Accumulate B if it's atomic, or flatten it if it's a plus expression
    (atomic(B) ->
        LeftB = B
    ;
        left_plus(B, LeftB)
    ),
    % Merge LeftA and LeftB into a new left plus expression
