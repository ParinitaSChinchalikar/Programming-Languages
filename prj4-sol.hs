...
itemsTotal items = sum $ map itemCost items
  where itemCost item = fromIntegral (itemNUnits item) * itemUnitPrice item
...
-- the factorial of n.
-- Restriction: May NOT use recursion of list comprehension
...
assoc _ [] = Nothing  -- empty list, key not found
assoc key ((k,v):xs) 
    | key == k = Just v  -- key found
    | otherwise = assoc key xs  -- continue searching recursively
...
substTerm (Var var) bindings = case assoc var bindings of
...
normalizeBindings bindings =
  let sortedBindings = sortBy (\(v1, _) (v2, _) -> compare v1 v2) bindings
  in map (\(var, term) -> (var, substTerm term sortedBindings)) sortedBindings
...
unify t1 t2 = normalizeBindings <$> unify' t1 t2 []

unify' :: Term -> Term -> [Binding] -> Maybe [Binding]
unify' (Var x) (Var y) bs
  | x == y = Just bs
  | otherwise =
      case lookup x bs of
        Just t' -> unify' t' (Var y) bs
        Nothing ->
          case lookup y bs of
            Just t' -> unify' (Var x) t' bs
            Nothing -> Just ((x, Var y) : map (substBinding x (Var y)) bs)
unify' (Var x) t bs =
  case lookup x bs of
    Just t' -> unify' t' t bs
    Nothing -> Just ((x, t) : map (substBinding x t) bs)
unify' t (Var x) bs =
  case lookup x bs of
    Just t' -> unify' t t' bs
    Nothing -> Just ((x, t) : map (substBinding x t) bs)
unify' (Struct a ts1) (Struct b ts2) bs
  | a /= b || length ts1 /= length ts2 = Nothing
...
substBinding :: VarName -> Term -> Binding -> Binding
substBinding x t (y, t') = (y, subst x t t')

subst :: VarName -> Term -> Term -> Term
subst x t (Var y) | x == y = t
subst _ _ t@(Var _) = t
subst x t (Struct name ts) = Struct name (map (subst x t) ts)
