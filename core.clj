...
(defn items-with-category1 [items category]
  (if (empty? items)
    ...
    (let [first-item (first items)
      ...
      (if (= category (:category first-item))
        (cons first-item (items-with-category1 rest-items category))
...
(defn items-total2 [items]
  (loop [items items
    ...
    (if (empty? items)
      ...
      (recur (rest items)
              (+ items-total2 (* (:unit-price item) (:n-units item)))
      )
      )
    )
  )
...
(defn items-total3 [items]
  (reduce 
    (fn [items-total3 item] (+ items-total3 (* (:n-units item) (:unit-price item)))) 
      0 items
  )
)
