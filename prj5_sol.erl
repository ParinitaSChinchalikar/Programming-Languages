...
(test_category_items1, enabled).
(test_category_items2, enabled).
(test_category_items3, enabled).
(test_delete_item, enabled).
(test_upsert_item, enabled).
(test_find_items, enabled).
(test_items_req, enabled).
(test_items_req_with_sort, enabled).
(test_items_client_no_sort, enabled).
(test_items_client_with_sort_dosort, enabled).
(test_items_client_with_sort, enabled).
(test_items_client_no_sort_mutate, enabled).
(test_items_client_with_sort_mutate, enabled).
(test_items_client_hot_reload, enabled).
...
category_items1(Category, [Item | Rest]) ->
    case Item#order_item.category == Category of
        true -> [Item | category_items1(Category, Rest)];
        false -> category_items1(Category, Rest)
    end.
...
delete_item(_Sku, []) -> [];
delete_item(Sku, [Item | Rest]) when Item#order_item.sku == Sku ->
    Rest;
delete_item(Sku, [Item | Rest]) ->
    [Item | delete_item(Sku, Rest)].
...
% [E] appended.
upsert_item(E, []) -> [E];
upsert_item(E1,[First|Last]) ->
  if E1#order_item.sku == First#order_item.sku ->
    [E1|Last];
  true ->
    [First|upsert_item(E1, Last)]
  end.
...
items_req({read, Sku}, Items) ->
    ...
    case find_items(Preds, Items) of
        ...
        _ -> {err, io_lib:format("Item with SKU ~p not found", [Sku]), Items}
    end;
...
items_req_with_sort({sort}, Items) ->
    {ok, void, lists:keysort(2,  Items)};
items_req_with_sort(Req, Items) ->
    case items_req(Req, Items) of
        {ok, Result, ItemsZ} -> {ok, Result, ItemsZ};
        {err, ErrString, ItemsZ} -> {err, ErrString, ItemsZ}
    end.
...
start_items_server(_Items, _Fn) -> 
Pid = spawn(fun() -> server_loop(_Items, _Fn) end),
register(items, Pid),
Pid.

server_loop(_Items, _Fn) ->
receive
{Pid, {stop}} ->
Pid ! {self(), {ok, stopped}},
ok;
{Pid, {new_fn, Fn1}} ->
Pid ! {self(), {ok, void}},
server_loop(_Items, Fn1);
{Pid, Req} ->
{Status, Result, NewItems} = _Fn(Req, _Items),
Pid ! {self(), {Status, Result}},
server_loop(NewItems, _Fn)
end.
...
stop_items_server() -> 
ServerPid = whereis(items),
ServerPid ! {self(), {stop}},
receive
{ServerPid, {ok, stopped}} ->
ok
end.
